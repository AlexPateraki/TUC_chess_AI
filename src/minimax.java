...
public class minimax {
	...
	public minimax(String[][] currentboard){
	...
	public String minimax_function(ArrayList<String> availableMoves, int depth, int maximizingPlayer) {		
		if(maximizingPlayer==1){//black nodes
			...
			int val = min_Value(minimax_World, depth);
			...
			int val2 = max_Value(minimax_World, depth);
	...
	public int min_Value(World world, int depth){ //if myColor = 1 I am the black player 
		...
		while(!queueB.isEmpty())
		{
		    ...
		    while (!nd_incr_PriorQ.isEmpty())
			{
				...
				eval = max_Value(newWorld, depth-1);
				...
				//int val = evaluation_function(this.blackPoints.get(counterB), this.whitePoints.getLast(),1,0);
	...
	public int max_Value(World world, int depth){//if myColor = 0 I am the white player 
		if (depth==0 || world.checkEnd()==true) {
		...
		while(!queueW.isEmpty())
		{
		    ...
		    while (!nd_decr_PriorQ.isEmpty())
			{
				...
				eval = min_Value(newWorld, depth-1);
				//move back
				...
				//int val = evaluation_function(this.whitePoints.get(counterW), this.blackPoints.getLast(),0,1);

				child.value = eval;
				nd_decr.value = max(nd_decr.value,child.points);
...
/*	private int calc_points(World newWorld, int row_ofBoard, int col_ofBoard) {
	...
	*/
	...
	public int find_pieces(World world, int player) {
		int pieces=0;
		if(player==1) {
			for (int i=0; i<world.rows; i++)
			{
				for (int j=0; i<world.columns; j++) {
					if(world.getBoard()[i][j]=="BK"||world.getBoard()[i][j]=="BP"||world.getBoard()[i][j]=="BR") {
						pieces++;
					}
				}
			}
			...
			{
				for (int j=0; i<world.columns; j++) {
					if(world.getBoard()[i][j]=="WK"||world.getBoard()[i][j]=="WP"||world.getBoard()[i][j]=="WR") {
						pieces++;
					}
				}
			}
	...
	public int evaluation_function(int points1, int points2, World world, int player1, int player2) {
		int pieces1 = find_pieces(world , player1);
		int pieces2 = find_pieces(world , player2);
		return points1 + pieces1  - (points2 + pieces2) ;
	}
